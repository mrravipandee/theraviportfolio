  ...
  return (
      <nav className="absolute top-0 left-0 w-full px-6 md:px-10 py-8 flex justify-between items-center z-50">
        ...
        <div className="hidden md:flex gap-10 font-mono text-[11px] tracking-[0.3em] uppercase text-[#1A1A1A]">
          {navLinks.map((link) => (
            <Link key={link.name} href={link.href} className="hover:opacity-50 transition">
              {link.name}
            </Link>
          ))}
        </div>
        ...
        >
          <div className={`w-6 h-[1px] bg-black transition-transform duration-300 ${isOpen ? "rotate-45 translate-y-[7px]" : ""}`} />
          <div className={`w-6 h-[1px] bg-black transition-opacity duration-300 ${isOpen ? "opacity-0" : ""}`} />
          <div className={`w-6 h-[1px] bg-black transition-transform duration-300 ${isOpen ? "-rotate-45 -translate-y-[7px]" : ""}`} />
        </button>
