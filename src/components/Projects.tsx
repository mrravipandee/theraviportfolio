    ...
    <section className="view-container pt-48 md:pt-64 mb-40 px-6 md:px-10 min-h-screen bg-[#F5F2EE]">
      ...
      <div className="border-b border-black/10 pb-12 mb-24">
        ...
        <h1 className="text-6xl md:text-8xl lg:text-9xl font-medium tracking-tighter leading-none">
          All <span className="italic font-light text-[#1A1A1A]">Products.</span>
        </h1>
      </div>
      ...
      <div className="flex flex-col gap-40">
        {projects.map((project, index) => (
          <div key={project.id || index} className="group flex flex-col md:flex-row gap-12 md:gap-24 items-start">
            ...
            <div className="w-full md:w-1/3 space-y-8 order-2 md:order-1">
              ...
              {project.technologies && (
                <div className="flex flex-wrap gap-2 pt-4">
                  {project.technologies.map((tech, i) => (
                    <span key={i} className="text-[10px] font-mono border border-black/10 px-2 py-1 uppercase tracking-tighter text-[#8C8680]">
                      {tech}
                    </span>
                  ))}
                </div>
              ...
              <div className="pt-8 flex gap-8 text-[11px] font-bold uppercase tracking-widest">
                <Link href={project.sourceLink} target="_blank" className="border-b-2 border-black pb-1 hover:opacity-50 transition-all">
                  GitHub Repository
                </Link>
                {project.liveLink && project.liveLink !== "#" && (
                  <Link href={project.liveLink} target="_blank" className="border-b border-black/20 pb-1 hover:border-black transition-all">
                    Live Demo
                  </Link>
                )}
              </div>
